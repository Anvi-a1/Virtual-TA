---
title: "AI coding in the CLI"
original_url: "https://tds.s-anand.net/#/ai-coding-cli?id=advanced-features"
downloaded_at: "2025-11-17T01:53:51.890323"
---
[AI Coding in the CLI](#/ai-coding-cli?id=ai-coding-in-the-cli)
===============================================================
...
[Claude Code CLI](#/ai-coding-cli?id=claude-code-cli)
-----------------------------------------------------
...
### [Basic usage](#/ai-coding-cli?id=basic-usage)
...
git diff | claude -p "Write a conventional commit message for this diff"Copy to clipboardErrorCopied
```

### [Key features](#/ai-coding-cli?id=key-features)
...
[Codex CLI (OpenAI)](#/ai-coding-cli?id=codex-cli-openai)
---------------------------------------------------------
...
### [Key features](#/ai-coding-cli?id=key-features-1)
...
### [Examples](#/ai-coding-cli?id=examples)
...
codex -c model_reasoning_effort=highCopy to clipboardErrorCopied
```

### [Slash commands (popular)](#/ai-coding-cli?id=slash-commands-popular)
...
[GitHub Copilot CLI](#/ai-coding-cli?id=github-copilot-cli)
-----------------------------------------------------------
...
### [Basic usage](#/ai-coding-cli?id=basic-usage-1)
...
COPILOT_MODEL=gpt-5 copilotCopy to clipboardErrorCopied
```

### [Key features](#/ai-coding-cli?id=key-features-2)
...
### [Slash commands (popular)](#/ai-coding-cli?id=slash-commands-popular-1)
...
[Gemini CLI](#/ai-coding-cli?id=gemini-cli)
-------------------------------------------
...
### [Core workflow](#/ai-coding-cli?id=core-workflow)
...
### [Examples](#/ai-coding-cli?id=examples-1)
...
gemini -p "$(jq -r .prompt prompt.json)"Copy to clipboardErrorCopied
```

### [Security and tool execution](#/ai-coding-cli?id=security-and-tool-execution)
...
### [Slash commands (popular)](#/ai-coding-cli?id=slash-commands-popular-2)
...
[Simon Willison’s `llm`](#/ai-coding-cli?id=simon-willisons-llm)
----------------------------------------------------------------
...
### [Core philosophy](#/ai-coding-cli?id=core-philosophy)
...
### [Essential commands](#/ai-coding-cli?id=essential-commands)
...
llm "Extract contact information as JSON" -a business_card.jpg --schema "name,email,phone"Copy to clipboardErrorCopied
```

### [Advanced features](#/ai-coding-cli?id=advanced-features)
...
cat auth.py | llm -t code-reviewCopy to clipboardErrorCopied
```
...
' -T grep 'Find TODOs in this repo and summarize'Copy to clipboardErrorCopied
```
...
llm -f large-codebase "Optimize the authentication logic"Copy to clipboardErrorCopied
```

### [Logging and caching](#/ai-coding-cli?id=logging-and-caching)
...
llm logs -c --tools llm_timeCopy to clipboardErrorCopied
```
...
diff -u \
  ...
  <(llm -t code-review -m anthropic:claude-3-5-sonnet < auth.py)Copy to clipboardErrorCopied
```

### [Real-world automation examples](#/ai-coding-cli?id=real-world-automation-examples)
...
llm "Analyze this performance profile and suggest optimizations" < perf_report.txtCopy to clipboardErrorCopied
```

### [Plugin ecosystem](#/ai-coding-cli?id=plugin-ecosystem)
...
llm install llm-gemini     # Google modelsCopy to clipboardErrorCopied
```
...
[Tips and best practices](#/ai-coding-cli?id=tips-and-best-practices)
---------------------------------------------------------------------

### [Standard CLI hygiene](#/ai-coding-cli?id=standard-cli-hygiene)
...
pytest --tb=short | llm "Explain test failures and suggest solutions"Copy to clipboardErrorCopied
```
...
git add analysis_*.mdCopy to clipboardErrorCopied
```
...
### [Cost and telemetry monitoring](#/ai-coding-cli?id=cost-and-telemetry-monitoring)
...
# Cost analysis
llm logs --since "last week" | awk '{sum+=$tokens} END {print "Total tokens:", sum}'Copy to clipboardErrorCopied
```
...
### [Remote and background execution](#/ai-coding-cli?id=remote-and-background-execution)
...
screen -dmS ai-monitor llm --streaming "Monitor system logs" < /var/log/syslogCopy to clipboardErrorCopied
```

**Keep host clean**: Avoid installing large AI dependencies on your development machine by using containerized execution.

### [GitHub integration](#/ai-coding-cli?id=github-integration)
...
jobs:
  review:
    ...
    steps:
      ...
      - run: git diff | llm "Review this PR: summarize risks and changes" | gh pr comment --body-file -Copy to clipboardErrorCopied
```
...
### [CI/CD integration](#/ai-coding-cli?id=cicd-integration)
...
#!/bin/bash
git diff --cached | llm "Check for obvious bugs or issues" --exit-codeCopy to clipboardErrorCopied
```
...
### [Reproducible workflows](#/ai-coding-cli?id=reproducible-workflows)
...
llm templates save code-review "Review for bugs, performance, security"Copy to clipboardErrorCopied
```
...
AI coding in IDEs](#/ai-coding-ide)

[Next

Code context engineering](#/ai-coding-context)