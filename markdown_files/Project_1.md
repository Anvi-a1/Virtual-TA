---
title: "Project 1"
original_url: "https://tds.s-anand.net/#/project-llm-code-deployment?id=build"
downloaded_at: "2025-11-17T01:52:33.945675"
---
[Project: LLM Code Deployment](#/project-llm-code-deployment?id=project-llm-code-deployment)
============================================================================================
...
[Request](#/project-llm-code-deployment?id=request)
---------------------------------------------------
...
[Build](#/project-llm-code-deployment?id=build)
-----------------------------------------------
...
1. Host an API endpoint that accepts a JSON POST sent via:
   ...
   curl https://example.com/api-endpoint \
     ...
     -d '{"brief": "...", ...}'Copy to clipboardErrorCopied
   ```
...
[Revise](#/project-llm-code-deployment?id=revise)
-------------------------------------------------
...
[Evaluate](#/project-llm-code-deployment?id=evaluate)
-----------------------------------------------------
...
[Evaluation Script](#/project-llm-code-deployment?id=evaluation-script)
-----------------------------------------------------------------------
...
round2:
  ...
  - brief: Allow filtering by region via #region-filter, update #total-sales with the filtered sum, and set data-region on that element to the active choice.
    checks:
      ...
      - js: document.querySelector("#total-sales").dataset.region !== undefinedCopy to clipboardErrorCopied
```
...
round2:
  ...
  - brief: Display a live word count badge #markdown-word-count that updates after every render and formats numbers with Intl.NumberFormat.
    checks:
      ...
      - js: !!document.querySelector("script").textContent.includes("Intl.NumberFormat")Copy to clipboardErrorCopied
```
...
round2:
  ...
  - brief: Cache the last successful lookup in localStorage under "github-user-${seed}" and repopulate the form on load.
    checks:
      ...
      - js: !!document.querySelector("script").textContent.includes("localStorage.getItem(\"github-user-${seed}\"")Copy to clipboardErrorCopied
```
