...
original_url: "https://tds.s-anand.net/#/ai-coding-tools?id=github-automation"
downloaded_at: "2025-11-17T02:03:45.729565"
---
[AI Coding Tools](#/ai-coding-tools?id=ai-coding-tools)
=======================================================
...
[Agent configuration surfaces](#/ai-coding-tools?id=agent-configuration-surfaces)
---------------------------------------------------------------------------------
...
[Least-privilege automation](#/ai-coding-tools?id=least-privilege-automation)
-----------------------------------------------------------------------------
...
[GitHub automation](#/ai-coding-tools?id=github-automation)
-----------------------------------------------------------
...
gh pr checks --watchCopy to clipboardErrorCopied
```
...
[Communication hooks](#/ai-coding-tools?id=communication-hooks)
---------------------------------------------------------------
...
[Data wrangling pipelines](#/ai-coding-tools?id=data-wrangling-pipelines)
-------------------------------------------------------------------------
...
[Local databases & analytics](#/ai-coding-tools?id=local-databases-amp-analytics)
---------------------------------------------------------------------------------
...
[Python environment hygiene](#/ai-coding-tools?id=python-environment-hygiene)
-----------------------------------------------------------------------------
...
[Python code quality & tests](#/ai-coding-tools?id=python-code-quality-amp-tests)
---------------------------------------------------------------------------------
...
uv run pytest -q
uv run mypy src
uv run pyright --outputjsonCopy to clipboardErrorCopied
```
...
[HTTP & web automation](#/ai-coding-tools?id=http-amp-web-automation)
---------------------------------------------------------------------
...
[Watchers & task runners](#/ai-coding-tools?id=watchers-amp-task-runners)
-------------------------------------------------------------------------
...
[Repository navigation & search](#/ai-coding-tools?id=repository-navigation-amp-search)
---------------------------------------------------------------------------------------
...
[Shell-native AI clients](#/ai-coding-tools?id=shell-native-ai-clients)
-----------------------------------------------------------------------
...
llm embed-multi --model text-embedding-3-large --to sqlite eval.db crash_notes/Copy to clipboardErrorCopied
```
...
[Multi-agent orchestration](#/ai-coding-tools?id=multi-agent-orchestration)
---------------------------------------------------------------------------
...
[JavaScript & browser automation](#/ai-coding-tools?id=javascript-amp-browser-automation)
-----------------------------------------------------------------------------------------
...
[Cloud automation](#/ai-coding-tools?id=cloud-automation)
---------------------------------------------------------
...
gcloud workflows run nightly-agent --data @params.json --location us-central1Copy to clipboardErrorCopied
```

Whitelist only the services you need, and pair with IAM policies scoped to staging projects.

[Ingestion & context builders](#/ai-coding-tools?id=ingestion-amp-context-builders)
-----------------------------------------------------------------------------------
...
[Tool recipes](#/ai-coding-tools?id=tool-recipes)
-------------------------------------------------
...
curl -s https://docs.example.com/api.html \
  | pandoc -f html -t gfm \
  | llm --save doc-ingestCopy to clipboardErrorCopied
```

[Model Context Protocol (MCP)](#/ai-coding-tools?id=model-context-protocol-mcp)
-------------------------------------------------------------------------------
...
### [Core MCP concepts](#/ai-coding-tools?id=core-mcp-concepts)
...
### [Safe tool integration](#/ai-coding-tools?id=safe-tool-integration)
...
### [Common MCP servers for development](#/ai-coding-tools?id=common-mcp-servers-for-development)

**SQLite Server**: Safe database access with read-only or limited write permissions

```
npx @mcp/server-sqlite ./project.db --read-onlyCopy to clipboardErrorCopied
```

**Memory Server**: Persistent context across AI sessions

```
npx @mcp/server-memory --storage ./ai-memory.jsonCopy to clipboardErrorCopied
```

**Sequential Thinking Server**: Enhanced reasoning capabilities

```
npx @mcp/server-sequential-thinkingCopy to clipboardErrorCopied
```

**File System Server**: Controlled file access with path restrictions

```
npx @mcp/server-filesystem --root ./src --readonlyCopy to clipboardErrorCopied
```

### [Integration patterns](#/ai-coding-tools?id=integration-patterns)
...
- name: Run AI Code Review
  run: |
    npx @mcp/server-github-api | llm "Review this PR for issues"Copy to clipboardErrorCopied
```
...
# Log analysis
npx @mcp/server-filesystem logs/ | llm "Find error patterns in recent logs"Copy to clipboardErrorCopied
```
...
[Summary](#/ai-coding-tools?id=summary)
---------------------------------------
...
- `slack chat send --channel dev-ai --text "$(git rev-parse --short HEAD) tests passed"` — ping maintainers when work is ready for review.Copy to clipboardErrorCopied
```

[Previous

Tests in AI coding](#/ai-coding-tests)
