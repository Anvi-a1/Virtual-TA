...
original_url: "https://tds.s-anand.net/#/llm-website-scraping?id=making-scraping-repeatable"
downloaded_at: "2025-11-17T02:02:10.604035"
---
...
[Why LLMs for web scraping?](#/llm-website-scraping?id=why-llms-for-web-scraping)
---------------------------------------------------------------------------------
...
[Vibe scraping workflow](#/llm-website-scraping?id=vibe-scraping-workflow)
--------------------------------------------------------------------------

The core principle: **outcomes over implementation**.

### [Traditional scraping](#/llm-website-scraping?id=traditional-scraping)
...
for product in products:
    ...
    data.append({'title': title, 'price': price})Copy to clipboardErrorCopied
```

### [Vibe scraping](#/llm-website-scraping?id=vibe-scraping)

```
Search for "AI website traffic" and scrape the content.
Convert into usable data that I can visualize.Copy to clipboardErrorCopied
```
...
[Chrome Remote Debugging setup](#/llm-website-scraping?id=chrome-remote-debugging-setup)
----------------------------------------------------------------------------------------
...
### [Step 1: Launch Chrome in debug mode](#/llm-website-scraping?id=step-1-launch-chrome-in-debug-mode)

**Windows:**

```
"C:\Program Files\Google\Chrome\Application\chrome.exe" --remote-debugging-port=9222 --user-data-dir="C:\chrome-debug-profile"Copy to clipboardErrorCopied
```

**Mac:**

```
/Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome --remote-debugging-port=9222 --user-data-dir="/tmp/chrome-debug-profile"Copy to clipboardErrorCopied
```

**Linux:**

```
google-chrome --remote-debugging-port=9222 --user-data-dir="/tmp/chrome-debug-profile"Copy to clipboardErrorCopied
```

This opens a new Chrome window with remote control enabled.

### [Step 2: Verify debugging works](#/llm-website-scraping?id=step-2-verify-debugging-works)

Open `http://localhost:9222` in the debug Chrome window. You should see a JSON list of open tabs.

### [Step 3: Give context to your coding agent](#/llm-website-scraping?id=step-3-give-context-to-your-coding-agent)
...
[Your scraping task here]Copy to clipboardErrorCopied
```
...
[Authenticated website scraping](#/llm-website-scraping?id=authenticated-website-scraping)
------------------------------------------------------------------------------------------

One of the most powerful LLM scraping use cases: extracting data from sites behind login walls.

### [Example: LinkedIn invites analysis](#/llm-website-scraping?id=example-linkedin-invites-analysis)
...
- Title
- Company
- Industry classificationCopy to clipboardErrorCopied
```
...
### [Example: Salesforce data extraction](#/llm-website-scraping?id=example-salesforce-data-extraction)
...
Save as salesforce_data.json.Copy to clipboardErrorCopied
```
...
### [Security considerations](#/llm-website-scraping?id=security-considerations)
...
[Handling dynamic content](#/llm-website-scraping?id=handling-dynamic-content)
------------------------------------------------------------------------------

Modern websites use JavaScript heavily. LLMs can handle this through browser automation.

### [Scenario 1: Hover-triggered content](#/llm-website-scraping?id=scenario-1-hover-triggered-content)

```
On this page, some data appears only when hovering over elements.
Scrape all visible data AND all hover-revealed content.Copy to clipboardErrorCopied
```
...
### [Scenario 2: Infinite scroll](#/llm-website-scraping?id=scenario-2-infinite-scroll)

```
This page uses infinite scroll. Scrape all items, not just
the initially visible ones. Keep scrolling until no new items appear.Copy to clipboardErrorCopied
```
...
# Now extract all itemsCopy to clipboardErrorCopied
```

### [Scenario 3: Lazy-loaded images](#/llm-website-scraping?id=scenario-3-lazy-loaded-images)

```
This page lazy-loads images as you scroll. Scrape all product
images, ensuring they're fully loaded before extraction.Copy to clipboardErrorCopied
```
...
[Multi-strategy scraping](#/llm-website-scraping?id=multi-strategy-scraping)
----------------------------------------------------------------------------

LLMs excel at trying multiple approaches when one fails.

### [Strategy 1: HTML parsing (fastest)](#/llm-website-scraping?id=strategy-1-html-parsing-fastest)
...
soup = BeautifulSoup(response.content, 'html.parser')Copy to clipboardErrorCopied
```

**Pros**: Fast, no browser overhead
**Cons**: Fails with JavaScript-rendered content

### [Strategy 2: API discovery (most reliable)](#/llm-website-scraping?id=strategy-2-api-discovery-most-reliable)
...
Use that API directly instead of scraping HTML.Copy to clipboardErrorCopied
```

**Pros**: Stable, structured data, faster than HTML
**Cons**: APIs may be authenticated or undocumented

### [Strategy 3: Browser automation (most powerful)](#/llm-website-scraping?id=strategy-3-browser-automation-most-powerful)
...
with sync_playwright() as p:
    ...
    page.goto(url)
    # Extract dataCopy to clipboardErrorCopied
```

**Pros**: Handles JavaScript, authentication, complex interactions
**Cons**: Slower, resource-intensive

### [Let the LLM choose](#/llm-website-scraping?id=let-the-llm-choose)
...
more complex approaches. Report which method succeeded.Copy to clipboardErrorCopied
```
...
[Extracting structured data](#/llm-website-scraping?id=extracting-structured-data)
----------------------------------------------------------------------------------

LLMs understand semantic structure, not just HTML structure.

### [Example: Product listings](#/llm-website-scraping?id=example-product-listings)

**Traditional scraping:**

```
products = soup.find_all('div', class_='product-grid-item')Copy to clipboardErrorCopied
```
...
### [Example: Tables with merged cells](#/llm-website-scraping?id=example-tables-with-merged-cells)

```
This page has a complex table with merged cells and nested headers.
Convert it to a flat CSV where each row is fully denormalized.Copy to clipboardErrorCopied
```
...
[Making scraping repeatable](#/llm-website-scraping?id=making-scraping-repeatable)
----------------------------------------------------------------------------------

Once a scraping session succeeds, convert it into reusable code.

### [Step 1: Successful scrape](#/llm-website-scraping?id=step-1-successful-scrape)
...
- Location
- Salary range
- Posted dateCopy to clipboardErrorCopied
```

### [Step 2: Extract as script](#/llm-website-scraping?id=step-2-extract-as-script)
...
Include a README.md with usage instructions.Copy to clipboardErrorCopied
```

### [Step 3: Schedule execution](#/llm-website-scraping?id=step-3-schedule-execution)

```
Create a GitHub Actions workflow that:
- Runs this scraper daily at 9 AM UTC
- Commits new jobs to the repo
- Sends email if scraping failsCopy to clipboardErrorCopied
```

**Result**: One-time vibe scraping becomes a production data pipeline.

[Error handling and debugging](#/llm-website-scraping?id=error-handling-and-debugging)
--------------------------------------------------------------------------------------

LLMs can debug scraping failures autonomously.

### [Scenario: Selector not found](#/llm-website-scraping?id=scenario-selector-not-found)

**Error:**

```
Element not found: .product-cardCopy to clipboardErrorCopied
```
...
Update the scraper and try again.Copy to clipboardErrorCopied
```
...
### [Scenario: Rate limiting](#/llm-website-scraping?id=scenario-rate-limiting)

**Error:**

```
HTTP 429 Too Many RequestsCopy to clipboardErrorCopied
```
...
- Resume from last successful pageCopy to clipboardErrorCopied
```

The LLM adds resilience without you writing retry logic.

### [Scenario: CAPTCHA](#/llm-website-scraping?id=scenario-captcha)
...
the CAPTCHA, then proceed with scraping.Copy to clipboardErrorCopied
```

LLMs can find workarounds (authenticated endpoints, API access) rather than attempting CAPTCHA solving.

[Comparing data across pages](#/llm-website-scraping?id=comparing-data-across-pages)
------------------------------------------------------------------------------------

LLMs excel at comparative scraping.

### [Example: Price comparison](#/llm-website-scraping?id=example-price-comparison)
...
Highlight best deals (>20% cheaper than alternatives).Copy to clipboardErrorCopied
```
...
[Monitoring and change detection](#/llm-website-scraping?id=monitoring-and-change-detection)
--------------------------------------------------------------------------------------------

Set up scrapers that alert you to changes.

### [Example: Competitor pricing](#/llm-website-scraping?id=example-competitor-pricing)
...
4. Maintains historical pricing in prices.dbCopy to clipboardErrorCopied
```
...
[Legal and ethical considerations](#/llm-website-scraping?id=legal-and-ethical-considerations)
----------------------------------------------------------------------------------------------

LLM scraping is powerful but comes with responsibilities.

### [When scraping is appropriate](#/llm-website-scraping?id=when-scraping-is-appropriate)
...
### [When to avoid scraping](#/llm-website-scraping?id=when-to-avoid-scraping)
...
### [Best practices](#/llm-website-scraping?id=best-practices)
...
[Tools and platforms](#/llm-website-scraping?id=tools-and-platforms)
--------------------------------------------------------------------

### [Coding agents for scraping](#/llm-website-scraping?id=coding-agents-for-scraping)
...
### [Supporting libraries](#/llm-website-scraping?id=supporting-libraries)
...
# Data extraction
import pandas as pd
import jsonCopy to clipboardErrorCopied
```

### [Debugging tools](#/llm-website-scraping?id=debugging-tools)
...
[Complete example: Job listing aggregator](#/llm-website-scraping?id=complete-example-job-listing-aggregator)
-------------------------------------------------------------------------------------------------------------

Full workflow from vibe scraping to deployed pipeline.

### [Initial prompt](#/llm-website-scraping?id=initial-prompt)
...
Handle pagination—get first 100 jobs from each site.Copy to clipboardErrorCopied
```

### [Refinement prompt](#/llm-website-scraping?id=refinement-prompt)
...
Create scraper.py that does all of this.Copy to clipboardErrorCopied
```

### [Production prompt](#/llm-website-scraping?id=production-prompt)
...
README.md
DEPLOYMENT.mdCopy to clipboardErrorCopied
```

**Result**: A fully automated job tracking system built entirely through natural language prompting.

[Best practices summary](#/llm-website-scraping?id=best-practices-summary)
--------------------------------------------------------------------------
...
[Real-world use cases](#/llm-website-scraping?id=real-world-use-cases)
----------------------------------------------------------------------

### [Data journalism](#/llm-website-scraping?id=data-journalism)
...
voting blocs.Copy to clipboardErrorCopied
```

### [Competitive intelligence](#/llm-website-scraping?id=competitive-intelligence)
...
Maintain historical pricing in Postgres.Copy to clipboardErrorCopied
```

### [Real estate analysis](#/llm-website-scraping?id=real-estate-analysis)
...
Identify underpriced properties (>20% below neighborhood median).Copy to clipboardErrorCopied
```

### [Academic research](#/llm-website-scraping?id=academic-research)
...
Analyze trending topics over time.Copy to clipboardErrorCopied
```

[Troubleshooting common issues](#/llm-website-scraping?id=troubleshooting-common-issues)
----------------------------------------------------------------------------------------

### [Issue 1: “Element not found”](#/llm-website-scraping?id=issue-1-element-not-found)
...
element with selector .product-card to appear before scraping.Copy to clipboardErrorCopied
```

### [Issue 2: “403 Forbidden”](#/llm-website-scraping?id=issue-2-403-forbidden)
...
Referer: [previous page URL]Copy to clipboardErrorCopied
```

### [Issue 3: Scraped data is incomplete](#/llm-website-scraping?id=issue-3-scraped-data-is-incomplete)
...
in Network tab that loads this data. Call that API directly.Copy to clipboardErrorCopied
```

### [Issue 4: Scraper works once, then fails](#/llm-website-scraping?id=issue-4-scraper-works-once-then-fails)
...
4. Add monitoring that alerts on failureCopy to clipboardErrorCopied
```

[Further resources](#/llm-website-scraping?id=further-resources)
----------------------------------------------------------------

* [Web Scraping with JavaScript tutorial](#/scraping-imdb-with-javascript)
* [Chrome DevTools guide](#/devtools)
