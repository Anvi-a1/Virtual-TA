---
title: "Tests in AI coding"
original_url: "https://tds.s-anand.net/#/ai-coding-tests?id=ai-coding-tests"
downloaded_at: "2025-11-17T01:53:04.937034"
---
[AI Coding Tests](#/ai-coding-tests?id=ai-coding-tests)
=======================================================
...
[Build evaluation loops](#/ai-coding-tests?id=build-evaluation-loops)
---------------------------------------------------------------------
...
* **Automate the selector harness**: When using Mixture-of-Agents or multiple drafts, promote the version that clears the scorecard. A simple shell script works:
  ...
  jq -s '{winner: max_by(.score)}' variations/*/result.jsonCopy to clipboardErrorCopied
  ```
...
[Premortem tests first](#/ai-coding-tests?id=premortem-tests-first)
-------------------------------------------------------------------
...
def test_rejects_missing_email(tmp_path: Path) -> None:
    with pytest.raises(ValueError):
        load_users(TEST_DATA.with_suffix(".bad"))Copy to clipboardErrorCopied
```
...
[Property-based fuzzing for AI patches](#/ai-coding-tests?id=property-based-fuzzing-for-ai-patches)
---------------------------------------------------------------------------------------------------
...
def test_slugify_is_idempotent(text: str) -> None:
    ...
    assert slug.encode("ascii", errors="ignore").decode() == slugCopy to clipboardErrorCopied
```

Run it fast:

```
uv run pytest tests/test_slugify_props.py -qCopy to clipboardErrorCopied
```
...
[Coverage and mutation guardrails](#/ai-coding-tests?id=coverage-and-mutation-guardrails)
-----------------------------------------------------------------------------------------
...
uv run mutmut resultsCopy to clipboardErrorCopied
```
...
[Hidden adversarial checks](#/ai-coding-tests?id=hidden-adversarial-checks)
---------------------------------------------------------------------------
...
[End-to-end smoke bots](#/ai-coding-tests?id=end-to-end-smoke-bots)
-------------------------------------------------------------------
...
* **Automate from the CLI**:
  ...
  uv run playwright test tests/e2e/smoke.spec.ts --headed=false --repeat-each=2Copy to clipboardErrorCopied
  ```
...
[LLM-assisted grading (with guardrails)](#/ai-coding-tests?id=llm-assisted-grading-with-guardrails)
---------------------------------------------------------------------------------------------------
...
[Operational checklist](#/ai-coding-tests?id=operational-checklist)
-------------------------------------------------------------------
