---
title: "Vibe Analysis"
original_url: "https://tds.s-anand.net/#/vibe-analysis?id=style-transfer-technique"
downloaded_at: "2025-11-17T02:02:53.799901"
---
[Vibe Analysis](#/vibe-analysis?id=vibe-analysis)
=================================================
...
[The traditional data science workflow](#/vibe-analysis?id=the-traditional-data-science-workflow)
-------------------------------------------------------------------------------------------------
...
[What’s dying vs what’s rising](#/vibe-analysis?id=whats-dying-vs-whats-rising)
-------------------------------------------------------------------------------

### [Tasks being automated](#/vibe-analysis?id=tasks-being-automated)
...
### [Skills becoming critical](#/vibe-analysis?id=skills-becoming-critical)
...
[Automated hypothesis generation](#/vibe-analysis?id=automated-hypothesis-generation)
-------------------------------------------------------------------------------------
...
Generate hypotheses and test them.Copy to clipboardErrorCopied
```
...
[Vibe analysis workflow](#/vibe-analysis?id=vibe-analysis-workflow)
-------------------------------------------------------------------

The complete workflow for vibe analysis:

### [1. Provide context](#/vibe-analysis?id=_1-provide-context)
...
- What prices people will happily payCopy to clipboardErrorCopied
```

### [2. Request comprehensive analysis](#/vibe-analysis?id=_2-request-comprehensive-analysis)
...
Summarize insights as poetry for entertainment.Copy to clipboardErrorCopied
```
...
### [3. Let it work autonomously](#/vibe-analysis?id=_3-let-it-work-autonomously)
...
### [4. Find non-obvious insights](#/vibe-analysis?id=_4-find-non-obvious-insights)

Have the LLM focus on surprising patterns:

```
Find the most interesting, useful, and non-obvious insights
from this data. Explain in simple, witty language for a senior exec.Copy to clipboardErrorCopied
```
...
[Automated data quality checks](#/vibe-analysis?id=automated-data-quality-checks)
---------------------------------------------------------------------------------
...
Suggest approaches to automatically fix these.Copy to clipboardErrorCopied
```
...
[Visual data stories](#/vibe-analysis?id=visual-data-stories)
-------------------------------------------------------------

Create beautiful, interactive visualizations styled after leading publications.

### [Style transfer technique](#/vibe-analysis?id=style-transfer-technique)

Instead of specifying every detail, reference a style:

```
Create a beautiful visual data story as a single-page web app
that explains these insights in the style of The New York Times.Copy to clipboardErrorCopied
```
...
### [Generate and publish](#/vibe-analysis?id=generate-and-publish)

Complete workflow:

```
Create a beautiful visual data story as a single-page web app
that explains this insight in the style of The New York Times.Copy to clipboardErrorCopied
```
...
# Open http://localhost:8000Copy to clipboardErrorCopied
```
...
[Creating synthetic data with hypotheses](#/vibe-analysis?id=creating-synthetic-data-with-hypotheses)
-----------------------------------------------------------------------------------------------------
...
5. Let me download as CSVCopy to clipboardErrorCopied
```
...
[Meta-prompting: improve prompts before execution](#/vibe-analysis?id=meta-prompting-improve-prompts-before-execution)
----------------------------------------------------------------------------------------------------------------------
...
```
Analyze this weekday vs weekend sales dataCopy to clipboardErrorCopied
```
...
Generate visualizations showing top/bottom performers and significant gaps.Copy to clipboardErrorCopied
```
...
[Parallel chat strategy](#/vibe-analysis?id=parallel-chat-strategy)
-------------------------------------------------------------------
...
[Model selection and reasoning](#/vibe-analysis?id=model-selection-and-reasoning)
---------------------------------------------------------------------------------
...
### [2. Checksum validation](#/vibe-analysis?id=_2-checksum-validation)
...
Create a checksum list.Copy to clipboardErrorCopied
```

This forces the LLM to validate its own work during generation.

### [3. Post-validation prompts](#/vibe-analysis?id=_3-post-validation-prompts)
...
- Impossible values
- Data quality issuesCopy to clipboardErrorCopied
```

[Making analysis repeatable](#/vibe-analysis?id=making-analysis-repeatable)
---------------------------------------------------------------------------
...
and generate an XLSX file with required data.Copy to clipboardErrorCopied
```
...
[Context management and session hygiene](#/vibe-analysis?id=context-management-and-session-hygiene)
---------------------------------------------------------------------------------------------------

### [Reset over repair](#/vibe-analysis?id=reset-over-repair)
...
### [Summarize and restart](#/vibe-analysis?id=summarize-and-restart)

After a long session with multiple corrections:

```
Given this conversation, give me one prompt that will do
the equivalent of all we have done so far.Copy to clipboardErrorCopied
```

Use this condensed prompt to start fresh, avoiding accumulated errors.

[Meta-analysis: analyzing your analysis](#/vibe-analysis?id=meta-analysis-analyzing-your-analysis)
--------------------------------------------------------------------------------------------------
...
[Risk mitigation strategies](#/vibe-analysis?id=risk-mitigation-strategies)
---------------------------------------------------------------------------

Three layers of protection when shipping LLM insights to executives:

### [1. Frame safe questions](#/vibe-analysis?id=_1-frame-safe-questions)
...
### [2. Reduce error likelihood](#/vibe-analysis?id=_2-reduce-error-likelihood)
...
### [3. Post-validation](#/vibe-analysis?id=_3-post-validation)
...
[Daily practice recommendations](#/vibe-analysis?id=daily-practice-recommendations)
-----------------------------------------------------------------------------------
...
[Complete example: aviation analysis](#/vibe-analysis?id=complete-example-aviation-analysis)
--------------------------------------------------------------------------------------------
...
- Downloadable data and analysis.py scriptCopy to clipboardErrorCopied
```

The LLM handles everything: data loading, cleaning, analysis, visualization, code generation, and deployment scaffolding.

[Tools and platforms](#/vibe-analysis?id=tools-and-platforms)
-------------------------------------------------------------
...
[Best practices summary](#/vibe-analysis?id=best-practices-summary)
-------------------------------------------------------------------
...
[The new data scientist role](#/vibe-analysis?id=the-new-data-scientist-role)
-----------------------------------------------------------------------------
...
[Further resources](#/vibe-analysis?id=further-resources)
---------------------------------------------------------
